...
func (s *Service) GetMonthlyTrends(monthsBack int) ([]MonthlyTrend, error) {
	...
	rows, err := s.db.Query(database.SelectMonthlyTrends, startDate.Format("2006-01-02"))
	if err != nil {
...
func (s *Service) GetVarianceReport(startDate, endDate string) ([]VarianceReport, error) {
	...
	rows, err := s.db.Query(database.SelectVarianceReport, startDate, endDate)
	if err != nil {
...
func (s *Service) GetCapacityUtilization(startDate, endDate string) ([]CapacityWeek, error) {
	...
	rows, err := s.db.Query(database.SelectCapacityUtilization, startDate, endDate)
	if err != nil {
