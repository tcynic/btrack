...
func (s *Service) UpdateActualHours(input models.UpdateActualHoursInput) (*models.WeeklyEntryWithStatus, error) {
	...
	err := s.db.QueryRow(`SELECT week_start_date FROM weekly_entries WHERE id = ?`, input.EntryID).Scan(&weekStartDate)
	if err != nil {
	...
	result, err := s.db.Exec(database.UpdateActualHours, input.ActualHours, input.EntryID)
	if err != nil {
	...
	return s.getWeeklyEntry(input.EntryID)
}
...
func (s *Service) GetWeeklyEntriesByWeek(weekStartDate string) ([]models.WeeklyEntryWithProject, error) {
	...
	rows, err := s.db.Query(database.SelectWeeklyEntriesByWeek, weekStartDate)
	if err != nil {
...
func (s *Service) ensurePersistentEntries(weekStartDate string) error {
	...
	rows, err := s.db.Query("SELECT id FROM projects WHERE is_persistent = 1 AND is_active = 1 AND deleted_at IS NULL")
	if err != nil {
...
func (s *Service) getWeeklyEntry(entryID int64) (*models.WeeklyEntryWithStatus, error) {
	row := s.db.QueryRow(`
